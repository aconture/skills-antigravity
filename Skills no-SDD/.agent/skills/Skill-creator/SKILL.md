...
| {thisProject}-specific | `{thisProject}-{component}` | `{thisProject}-api`, `{thisProject}-ui`, `{thisProject}-sdk-check` |
| Testing skill | `{thisProject}-test-{component}` | `{thisProject}-test-sdk`, `{thisProject}-test-api` |
...
## Decision: {thisProject}-Specific vs Generic

```
Patterns apply to ANY project?     → Generic skill (e.g., pytest, typescript)
Patterns are {thisProject}-specific?     → {thisProject}-{name} skill
Generic skill needs {thisProject} info?  → Add references/ pointing to {thisProject} docs
```
...
| `license` | Yes | Always `Apache-2.0` for {thisProject} |
| `metadata.author` | Yes | `AGCC` |
